...
import { FileText, MapPin, Clock, Calendar, Search, Sparkles, ArrowRight } from "lucide-react";
...
export default async function BrowseTendersPage({
  ...
  const t = await getTranslations("BrowseTenders");
  ...
  try {
    ...
    // Fetch tenders from API (public endpoint, no auth required)
    const apiTenders = await apiClient.getTenders(apiParams);
    tenders = Array.isArray(apiTenders)
      ? apiTenders.filter((tender: any) => tender.status === "open")
      : [];
  } catch (error) {
    ...
    if (error instanceof Error && error.message.includes('Unable to connect')) {
      fetchError = 'Unable to connect to the server. Please ensure the backend API is running.';
    } else {
      fetchError = 'Failed to load tenders. Please try again later.';
    }
    tenders = [];
  }
                  ...
                  >
                    ...
                    <CardHeader className="relative z-10">
                      ...
                      >
                        <div className={`flex-1 ${isRTL ? "text-right" : "text-left"}`}>
                          ...
                          >
                            ...
                            >
                              ...
                              <span className="font-semibold text-foreground">{tender.organization?.name || t("organization")}</span>
                            </span>
                            {tender.location && (
                              <span
                                className={`flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}
                        ...
                        {tender.deadline && (
                          <Badge
                            variant="secondary"
                            className={`rounded-full px-4 py-1.5 bg-gradient-to-r from-purple-50 to-pink-50 text-purple-700 border-2 border-purple-200/60 font-semibold text-sm shadow-sm flex items-center gap-1.5 ${isRTL ? "flex-row-reverse" : ""}`}
                          >
                            <Calendar className="h-3.5 w-3.5" />
                            <span>{t("deadline")}: {new Date(tender.deadline).toLocaleDateString()}</span>
                          </Badge>
                        )}
                      </div>
                    </CardHeader>
                    <CardContent className="relative z-10">
                      ...
                      >
                        ...
                        >
                          <Link href={`/browse/tenders/${tender.id}`} className="flex items-center gap-2">
                            {t("viewDetails")}
                            <ArrowRight className="h-4 w-4 group-hover/btn:translate-x-1 transition-transform duration-300" />
                          </Link>
                        </Button>
                  ...
                  <CardContent className={`py-20 text-center ${isRTL ? "text-right" : "text-left"}`}>
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="text-xl font-bold text-foreground mb-2">
                        {fetchError ? "Connection Error" : t("noTendersFound")}
                      </h3>
                      <p className="text-muted-foreground text-base max-w-md">
                        {fetchError || (
                          params.search
                            ? "Try adjusting your search terms or browse all available tenders."
                            : "Check back soon for new opportunities!"
                        )}
                      </p>
                      {fetchError && (
                        <p className="text-sm text-muted-foreground mt-2 max-w-md">
                          Make sure the backend server is running at <code className="bg-gray-100 px-2 py-1 rounded">http://localhost:3001</code>
                        </p>
                      )}
                    </div>
                  </CardContent>
